...
const NotificationPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        if (auth.currentUser) {
          ...
          const data = sortedNotifications?.map((notice) => notice as Notification) 
  ...
  return (
    <div className='w-full'>
      ...
      {isLoading ? <div className='text-center py-3 mt-3'><Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'start', height: '100vh' }}>
        <CircularProgress />
      </Box></div> :
